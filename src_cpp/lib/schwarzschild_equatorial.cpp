...
static inline double stable_dv_dtau(double M, double E, double L, double r, double pr) {
    ...
    double denom = (E - pr);
    if (!is_finite(denom) || std::abs(denom) < 1e-14) denom = (denom >= 0.0 ? 1e-14 : -1e-14);
    return B / denom;
}
...
static inline void rhs_schw_eq(
    ...
    const double A_safe = (A >= 1e-300) ? A : 1e-300;
    dt = E / A_safe;
    if (A > 1e-8 && std::abs(E + pr) > 1e-10) dv = (E + pr) / A_safe;
    else dv = stable_dv_dtau(M, E, L, r, pr);
...
TrajectorySchwarzschildEq simulate_schwarzschild_equatorial_rk4(
    ...
    TrajectorySchwarzschildEq traj;
    traj.M = M; traj.E = E; traj.L = L; traj.r0 = r0; traj.phi0 = phi0;
    traj.status = OrbitStatus::BOUND;

    const double h_step = cfg.dt;
    if (!(h_step > 0.0) || !is_finite(h_step) || !(tauf >= tau0) || !(M > 0.0)) {
    ...
    auto append_sample = [&](double tau_s, double r_s, double phi_s, double t_s, double v_s, double pr_s) {
        ...
        traj.epsilon.push_back(pr_s * pr_s + Veff_schw(M, r_s, L) - E * E);
    ...
    for (int step = 0; step < n_steps; ++step) {
        ...
        double k2_r, k2_phi, k2_t, k2_v, k2_pr; rhs_schw_eq(M, E, L, r + 0.5*h*k1_r, phi + 0.5*h*k1_phi, tcoord + 0.5*h*k1_t, vcoord + 0.5*h*k1_v, pr + 0.5*h*k1_pr, k2_r, k2_phi, k2_t, k2_v, k2_pr);
        double k3_r, k3_phi, k3_t, k3_v, k3_pr; rhs_schw_eq(M, E, L, r + 0.5*h*k2_r, phi + 0.5*h*k2_phi, tcoord + 0.5*h*k2_t, vcoord + 0.5*h*k2_v, pr + 0.5*h*k2_pr, k3_r, k3_phi, k3_t, k3_v, k3_pr);
        double k4_r, k4_phi, k4_t, k4_v, k4_pr; rhs_schw_eq(M, E, L, r + h*k3_r, phi + h*k3_phi, tcoord + h*k3_t, vcoord + h*k3_v, pr + h*k3_pr, k4_r, k4_phi, k4_t, k4_v, k4_pr);
        ...
        if (crossed_pr || crossed_rcap || crossed_hor) {
            ...
            rhs_schw_eq(M, E, L, r_next, phi_next, t_next, v_next, pr_next, dr_nx, dphi_nx, dt_nx, dv_nx, dpr_nx);
    ...
    for (size_t i = 0; i < N; ++i) {
        const double rr = traj.r[i], A = safe_A(M, rr);
        if (!is_finite(rr) || !(rr > 0.0) || A < 1e-12) continue;
        const double ut = E / A, ur = traj.pr[i], up = L / (rr * rr);        
        double vt = (A > 1e-8 && std::abs(E + ur) > 1e-10) ? (E + ur) / A : stable_dv_dtau(M, E, L, rr, ur);
        traj.ut_theory[i] = ut; traj.vt_theory[i] = vt; traj.ur_theory[i] = ur; traj.uphi_theory[i] = up;
        traj.norm_u_theory[i] = (-A)*(ut*ut) + (1.0/A)*(ur*ur) + (rr*rr)*(up*up) + 1.0;
    }
    return traj;
}
