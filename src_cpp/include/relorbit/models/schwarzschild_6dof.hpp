...
//     f^r   = (F/m) · n_r                           [unidades geométricas]
//     f^phi = (F/m) · n_phi / r
...
//     dL/dτ = r² · f^phi  = r · (F/m) · n_phi
//     dE/dτ = A(r)⁻¹ · E · f^t       (A = 1 − 2M/r)
//     dm/dτ = −m · |F|_physical / (Isp · g₀)
//
...
//   (útil para modelar offset de CM ou misalignment do motor)
struct EngineCfg {
    ...
    bool   active(double tau)   const { return tau >= tau_on && tau <= tau_off && F_newton > 0.0; }
    // Aceleração específica em unidades geométricas: a = (F/m) / c²
    double F_geom(double m_kg)  const { return (m_kg > 0.0) ? (F_newton / (m_kg * DOF6_C_MS * DOF6_C_MS)) : 0.0; }
};
