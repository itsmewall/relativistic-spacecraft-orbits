...
Uso típico (YAML, model: "schwarzschild_6dof"):
  - name: "Missao_ThrustVectoring"
    ...
    engine:
      ...
      mass0_kg: 1000.0
      dry_mass_kg: 300.0
      nozzle_body: [0, 0, 1]     # bocal no eixo +z do body frame
      tau_on:   0.0
      tau_off:  400.0
    ext_torque:
...
def _build_engine(d: Dict[str, Any]) -> Any:
    ...
    tr_list = eng.get("torque_reaction", [0.0, 0.0, 0.0])
    e.torque_reaction = [float(tr_list[0]), float(tr_list[1]), float(tr_list[2])]
    return e
...
def validate_6dof(result: Result6DOF) -> Dict[str, Any]:
    ...
    notes = [
        f"Orbita: r in [{s.get('r_range', ['?','?'])[0]:.2f}, {s.get('r_range',['?','?'])[1]:.2f}] M",
        ...
        f"T_rot drift   = {s.get('T_rot_rel_drift', float('nan')):.2e}",
    ]
    return {
